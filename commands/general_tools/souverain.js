 ...
 * Deux contacts Supreme Owner sous forme de vCard WhatsApp, chacun avec
 * newsletter/transfert et boutons sociaux fonctionnels.
 */
...
async function sendOwnerCard(sock, jid, number) {
  const buttons = buildButtons(number);
  const text =
    `╭─❑ *OWNER • THE BIG DIPPER* ❑─⚯\n` +
    `┃🌹 *${OWNER_NAME}*\n` +
    `┃📱 *+${number}*\n` +
  ...
  const generated = generateWAMessageFromContent(
    ...
    { interactiveMessage },
    {
      quoted: buildQuotedContact(jid, number),
  ...
  async execute(sock, msg, args, extra) {
    ...
    try {
      for (const number of OWNER_NUMBERS) {
        await sendOwnerCard(sock, jid, number);
      }
      try {
