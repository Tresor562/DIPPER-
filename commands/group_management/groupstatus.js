...
const GROUP_STATUS_WIRE_TAG = GROUP_STATUS_FIELD_NUMBER << 3; // bool => wire type 0

/**
 * Baileys 6.7.22 connait statusSourceType (champ 64), mais pas encore
 * isGroupStatus (champ 66). On ajoute uniquement ce champ à l'encodage
 * protobuf sortant pour garder tout le bot sur 6.7.22 sans migration majeure.
 */
...
async function downloadToBuffer(message, type) {
  ...
  if (!buffer.length) {
    throw new Error('Le média est vide ou a expiré.');
  }
...
async function postRealGroupStatus(sock, groupJid, sourceMessage, overrideCaption = '') {
  if (!groupJid?.endsWith('@g.us')) {
    throw new Error('Le destinataire n’est pas un groupe WhatsApp valide.');
  }

  const payload = await buildStatusPayload(sourceMessage, overrideCaption);
  if (!payload) {
    throw new Error('Aucun texte, image, vidéo, GIF, audio ou sticker compatible trouvé.');
  }

  const result = await sock.sendMessage(groupJid, payload.content);
  if (!result?.key?.id) {
    throw new Error('WhatsApp n’a pas confirmé la création du message de statut.');
  }
  ...
  async execute(sock, msg, args, extra) {
    ...
    if (!source) {
      ...
      if (ownHasMedia) {
        ...
        source = { conversation: suppliedText };
      }
    }
