...
module.exports = {
  ...
  async execute(sock, msg, args, extra) {
    ...
    try {
      ...
      const commandCalled = bodyText.trim().split(/\s+/)[0].replace(config.prefix || '.', '').toLowerCase();
      ...
      if (!action || !['on', 'off'].includes(action)) {
        ...
        return reply(`*╭━≪• ʀɪᴛᴜᴇʟs ᴅ'ᴀᴄᴄᴜᴇɪʟ •≫╾╮*\n*┃* 📊 *${toSmallCaps('statut')} :* ${status}\n*┃* 🔮 *${toSmallCaps('incantations')} :*\n*┃* ${prefix}accueil on / off\n\n*┃* 💡 *${toSmallCaps('astuce')} :* ${toSmallCaps('utilisez')} \`${prefix}setwelcome <message>\` ${toSmallCaps('pour personnaliser le texte d\'entree')}.\n\n${extra.phrases.footer()}`);
      }

      const enable = action === 'on';
      if (enable && groupSettings.welcome) return reply(`*⚠️ ${toSmallCaps('les rituels d\'accueil sont deja actifs')} !*\n\n${extra.phrases.footer()}`);
      if (!enable && !groupSettings.welcome) return reply(`*⚠️ ${toSmallCaps('les rituels d\'accueil sont deja endormis')} !*\n\n${extra.phrases.footer()}`);
