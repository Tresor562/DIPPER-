...
/**
 * THE BIG DIPPER — Global bilingual layer
 * Default: French. Optional: English.
 * Preference is isolated and persisted per WhatsApp session.
 */
const fs = require('fs');
...
const FALLBACK_REPLACEMENTS = [
  [/Commande inconnue/gi, 'Unknown command'],
  [/commandes disponibles/gi, 'available commands'],
  [/Traitement en cours/gi, 'Processing'],
  [/Patiente/gi, 'Please wait'],
  [/Accès refusé/gi, 'Access denied'],
  [/réservé aux/gi, 'reserved for'],
  [/Le bot est actuellement en mode silencieux/gi, 'The bot is currently muted'],
  [/Le bot est actuellement en mode privé/gi, 'The bot is currently in private mode'],
  [/Cette commande/gi, 'This command'],
  [/Informations système/gi, 'System information'],
  [/Infos système/gi, 'System info'],
  [/Téléchargements/gi, 'Downloads'],
  [/Outils généraux/gi, 'General tools'],
  [/Gestion de groupe/gi, 'Group management'],
  [/Langue actuelle/gi, 'Current language'],
  [/groupe/gi, 'group'],
  [/utilisateur/gi, 'user'],
  [/propriétaire/gi, 'owner'],
  [/administrateur/gi, 'administrator'],
  [/commande/gi, 'command'],
  [/Erreur/gi, 'Error'],
  [/Échec/gi, 'Failure'],
  [/Succès/gi, 'Success'],
...
const TRANSLATABLE_KEYS = new Set([
  'text', 'caption', 'title', 'subtitle', 'description', 'footer', 'footerText',
  'contentText', 'buttonText', 'displayText', 'selectedDisplayText', 'display_text',
  'body', 'header', 'label',
]);
...
async function translateNode(node, seen = new WeakSet(), keyHint = '') {
  ...
  for (const key of Object.keys(node)) {
    ...
    if (typeof value === 'string' && (TRANSLATABLE_KEYS.has(key) || key === 'buttonParamsJson')) {
      ...
      await translateNode(value, seen, key);
    }
  }
  return node;
}
...
async function installLanguageMiddleware(sock, sessionId = 'default') {
  ...
  if (typeof sock.sendMessage === 'function') {
    ...
    sock.sendMessage = async (jid, payload, options) => {
      ...
      const translated = lang === 'en' ? await translatePayload(payload, lang) : payload;
      return originalSendMessage(jid, translated, options);
    };
  }
  ...
  if (typeof sock.relayMessage === 'function') {
    ...
    sock.relayMessage = async (jid, message, options) => {
      const lang = sock.__botLanguage || getLanguage(id);
      const translated = lang === 'en' ? await translatePayload(message, lang) : message;
      return originalRelayMessage(jid, translated, options);
    };
  }
