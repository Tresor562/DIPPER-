...
class GameCenterEngine {
  ...
  startPrefer(chatId,by){ const error=this._startGuard(chatId,'prefer'); if(error)return {error}; const pick=PREFER_BANK[Math.floor(Math.random()*PREFER_BANK.length)]; const gid=id('prefer'); return this._put({id:gid,alias:short(gid),chatId,type:'prefer',status:'playing',by,choices:pick,votes:{},startedAt:now()}); }
  ...
  startChain(chatId,by){ const error=this._startGuard(chatId,'word-chain'); if(error)return {error}; const seed=CHAIN_WORDS[Math.floor(Math.random()*CHAIN_WORDS.length)]; const gid=id('chain'); return this._put({id:gid,alias:short(gid),chatId,type:'word-chain',status:'playing',by,lastWord:seed,used:[seed],turn:0,scores:{},history:[],startedAt:now()}); }
