...
function isPublicationMessage(msg) {
  ...
  return !!(m.conversation || m.extendedTextMessage || m.imageMessage || m.videoMessage || m.documentMessage || m.audioMessage || m.pollCreationMessage || m.stickerMessage);
}
...
function readLocalState(sessionId) {
  try {
    ...
    return { ids: Array.isArray(parsed?.recentServerIds) ? parsed.recentServerIds.map(String) : [], reactNext: typeof parsed?.reactNext === 'boolean' ? parsed.reactNext : true };
  } catch (_) { return { ids: [], reactNext: true }; }
}
...
function writeLocalState(sessionId, ids, reactNext) {
  try {
    ...
    fs.writeFileSync(file, JSON.stringify({ recentServerIds: ids.slice(-MAX_SEEN), reactNext, updatedAt: new Date().toISOString() }, null, 2));
  } catch (_) {}
}
...
async function persistState(sessionId, state) {
  ...
  if (db) {
    try {
      await db.collection(MONGO_COLLECTION).updateOne({ _id: mongoDocId(sessionId) }, { $set: { recentServerIds: ids, reactNext: state.reactNext, updatedAt: new Date() } }, { upsert: true });
    } catch (err) { console.warn(`[SecondaryChannelReact] ${sessionId}: persistance Mongo échouée: ${err.message}`); }
  }
}
...
async function subscribeToLiveUpdates(sock, jid, sessionId) {
  if (typeof sock.subscribeNewsletterUpdates !== 'function') return false;
  try {
...
async function reactWithRetry(sock, jid, serverId, emoji) {
  let lastError;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try { await sock.newsletterReactMessage(jid, serverId, emoji); return; }
    catch (err) { lastError = err; if (attempt < 3) await wait(1800 * attempt); }
  }
  throw lastError;
}
...
async function installSecondaryChannelAutoReact(sock, meta = {}) {
  ...
  if (!jid || !String(jid).endsWith('@newsletter')) return { enabled: false, reason: 'invalid_jid' };
  if (typeof sock.newsletterReactMessage !== 'function') return { enabled: false, reason: 'unsupported' };
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    ...
    for (const msg of messages || []) {
      if (msg?.key?.remoteJid !== jid || !msg.message || !isPublicationMessage(msg)) continue;
      const serverId = getServerId(msg);
      if (!serverId) continue;
      sock._dipperSecondaryChannelReactQueue = sock._dipperSecondaryChannelReactQueue.then(async () => {
        ...
        const baseEmoji = chooseReactionEmoji(extractPublicationText(msg), msg.message);
        const emoji = chooseSecondaryEmoji(baseEmoji, sessionId, serverId);
        await wait(2500 + (hash(`${serverId}:${sessionId}:delay`) % 4500));
        await reactWithRetry(sock, jid, serverId, emoji);
        console.log(`[SecondaryChannelReact] ✅ ${sessionId}: publication ${serverId} → ${emoji}`);
      }).catch(err => console.warn(`[SecondaryChannelReact] ⚠️ ${sessionId}: réaction ${serverId} échouée: ${String(err?.message || err).slice(0, 160)}`));
    }
  });

  console.log(`[SecondaryChannelReact] ✅ ${sessionId}: auto-réactions universelles activées → ${jid} | origin=${meta.origin || 'unknown'}`);
  return { enabled: true, jid, origin: meta.origin || 'unknown' };
}

module.exports = { installSecondaryChannelAutoReact, chooseSecondaryEmoji };
