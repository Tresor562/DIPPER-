 ...
 *
 * Objectif : une commande fournit son contenu ; le style actif décide de la
 * présentation. Ce module ne touche jamais à la logique métier des commandes.
 * Il sait aussi nettoyer les anciens cadres lourds au dernier moment, ce qui
 * permet une migration progressive sans régression fonctionnelle.
 */
...
// Toute la famille box-drawing + ornements utilisés par les anciens cadres.
const BOX_CHARS = /[\u2500-\u257f╼╾≪≫]/g;
const HEAVY_FRAME = /[╭╮╰╯┃║╔╗╚╝╠╣╦╩╬┌┐└┘│]/g;
...
function normalizeLine(line) {
  let out = String(line ?? '');
  // Retirer les bords de cadres sans toucher au texte informatif.
  out = out.replace(HEAVY_FRAME, ' ');
  out = out.replace(BOX_CHARS, ' ');
  // Retirer les restes d'ornements lourds aux extrémités uniquement.
...
function sanitizeLegacyText(text, style) {
  ...
  for (const original of lines) {
    ...
    // Une ligne purement décorative devient au plus UN séparateur court.
    ...
    // Les vieilles lignes "┃ texte" deviennent simplement "texte".
    clean = clean.replace(/^\s*[|:]+\s*/, '');
    out.push(clean);
  }

  // Espacement discipliné : max une ligne vide consécutive.
  const compact = [];
...
function decoratePayload(payload, style) {
  ...
  // Réactions, suppressions, stickers, etc. sont laissés exactement tels quels.
  if (payload.react || payload.delete) return payload;
  let changed = false;
  const next = { ...payload };
  if (typeof next.text === 'string') {
    const cleaned = sanitizeLegacyText(next.text, style);
    if (cleaned !== next.text) { next.text = cleaned; changed = true; }
  }
  if (typeof next.caption === 'string') {
    const cleaned = sanitizeLegacyText(next.caption, style);
