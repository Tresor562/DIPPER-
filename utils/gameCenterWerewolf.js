...
class WerewolfStore{
  ...
  role(alias,userId){const g=this._liveByAlias(alias);if(!g||!g.players.includes(userId))return null;return{role:g.roles[userId],alive:g.alive[userId],chatId:g.chatId,alias:g.alias,roster:rosterText(g),round:g.round,phase:g.phase};}
  nightAction(alias,userId,action,targetNumber,{randomInt=crypto.randomInt}={}){
    ...
    g.updatedAt=Date.now();this._save();const ready=this.nightReady(g);const seenIsWolf=action==='see'?g.roles[target]==='wolf':null;let resolution=null;if(ready)resolution=this.resolveNight(g.chatId,{randomInt});return{ok:true,seenIsWolf,target,ready,resolution,chatId:g.chatId,alias:g.alias};
  }
  ...
  forceResolve(chatId){const g=this._ensure().games[chatId];if(!g)return{error:'not-found'};if(g.phase==='night'){g.night.seerDone=true;g.night.doctorDone=true;g.updatedAt=Date.now();this._save();return this.resolveNight(chatId);}if(g.phase==='day')return this.resolveDay(chatId);return{error:'phase'};}
