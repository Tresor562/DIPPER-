...
// Vérifie si un utilisateur figure dans la liste blanche du groupe
// (commande .allow — bypass des protections automatiques de contenu)
// Utilise buildComparableIds pour matcher correctement PN et LID,
// comme le reste de ce fichier.
const isAllowedUser = (jid, groupSettings) => {
  if (!jid || !groupSettings) return false;
  const allowed = groupSettings.allowedUsers || [];
  if (!allowed.length) return false;

  const targets = buildComparableIds(jid);
  if (!targets.length) return false;

  return allowed.some(allowedJid => {
    const allowedTargets = buildComparableIds(allowedJid);
    return allowedTargets.some(id => targets.includes(id));
  });
};
