...
function extractPublicationText(msg) {
  ...
  return m.conversation || m.extendedTextMessage?.text || m.imageMessage?.caption || m.videoMessage?.caption || m.documentMessage?.caption || m.pollCreationMessage?.name || '';
}
...
async function persistState(state) {
  ...
  if (db) {
    try {
      await db.collection(MONGO_COLLECTION).updateOne({ _id: MONGO_DOC_ID }, { $set: { recentServerIds: ids, reactNext: state.reactNext, updatedAt: new Date() } }, { upsert: true });
    } catch (err) { console.warn('[ChannelReact] ⚠️ Persistance Mongo échouée:', err.message); }
  }
}
...
async function subscribeToLiveUpdates(sock, jid) {
  ...
  try {
    ...
    console.log('[ChannelReact] 📡 Live updates actifs');
    return true;
  } catch (err) {
...
async function installMainChannelAutoReact(sock) {
  ...
  if (!jid || !String(jid).endsWith('@newsletter')) return;
  if (typeof sock.newsletterReactMessage !== 'function') return;

  try { await require('./channelAutoFollow').ensureChannelFollow(sock, 'main'); } catch (_) {}
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    ...
    for (const msg of messages || []) {
      ...
      sock._dipperMainChannelReactQueue = sock._dipperMainChannelReactQueue.then(async () => {
        ...
        const emoji = chooseReactionEmoji(extractPublicationText(msg), msg.message);
        await wait(2200);
  ...
  console.log(`[ChannelReact] ✅ Auto-réactions actives sur le compte principal → ${jid}`);
}
