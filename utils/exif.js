...
async function writeExifVid(videoBuffer, metadata) {
  const { packname } = metadata;
  // Résolution robuste : ffmpeg-static en priorité, sinon ffmpeg système
  let ffmpegPath = 'ffmpeg';
  try {
    const sp = require('ffmpeg-static');
    if (sp && require('fs').existsSync(sp)) {
      require('fs').accessSync(sp, require('fs').constants.X_OK);
      ffmpegPath = sp;
    }
  } catch (_) {
    const sysPaths = ['/usr/bin/ffmpeg', '/usr/local/bin/ffmpeg', '/bin/ffmpeg'];
    for (const p of sysPaths) {
      try { require('fs').accessSync(p, require('fs').constants.X_OK); ffmpegPath = p; break; } catch (_) {}
    }
  }
