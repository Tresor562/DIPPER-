...
if(!helper.includes(MARKER)){
  const start=helper.indexOf('const SPECIAL_COMMANDS = new Set(['),end=start<0?-1:helper.indexOf(']);',start);if(start<0||end<0)throw new Error('[target-footer] SPECIAL_COMMANDS introuvable');
  const set=`const SPECIAL_COMMANDS = new Set([\n  // ${MARKER}\n  'menu', 'grimoire', 'allmenu', 'commands', 'index', 'menu2', 'help',\n  'ping', 'alive', 'uptime',\n]);`;helper=helper.slice(0,start)+set+helper.slice(end+3);
  const anchor=`  const { text = '', style = styleManager.getStyle(), imageBuffer = null, commandName = '' } = options;`;if(!helper.includes(anchor))throw new Error('[target-footer] options sendSpecialPresentation introuvables');
  ...
  const bodyAnchor=`body: proto.Message.InteractiveMessage.Body.create({ text: disciplineSpecialText(text) }),`;if(!helper.includes(bodyAnchor))throw new Error('[target-footer] body specialPresentation introuvable');helper=helper.replace(bodyAnchor,`body: proto.Message.InteractiveMessage.Body.create({ text: disciplineSpecialText(displayText) }),`);fs.writeFileSync(helperPath,helper,'utf8');
}
