...
// 1) Un feedback central robuste : groupe avec quote puis sans quote, plusieurs essais.
...
// 3) Aucun gate d'accès ne doit rester silencieux pour une commande explicite.
...
// 4) Si antispam/purification consomme une commande, l'utilisateur reçoit quand même un état clair.
...
// 5) Une commande inconnue doit répondre même si le moteur flou lui-même plante.
...
src = src.replace(
  ...
  '    if (commandResponseTrace.responses === 0) { // [NO SILENT noReply]'
);

// 7) Le catch global ne doit jamais planter sa propre réponse d'erreur.
...
// Les rate-limit ne doivent plus produire un silence immédiat : petit délai + feedback retry.
...
for (const required of [
  MARKER, FEEDBACK_MARKER, UNKNOWN_MARKER, SLOW_MARKER,
  '[BANNED COMMAND FEEDBACK]', '[MUTED COMMAND FEEDBACK]',
  '[NO SILENT noReply]', 'sendCommandFeedback(', 'const errText = errMsgs['
]) {
  if (!final.includes(required)) throw new Error('[command-guarantee] garde-fou absent: ' + required);
}
if (final.includes('commandResponseTrace.responses === 0 && command.noReply !== true')) {
  throw new Error('[command-guarantee] noReply peut encore créer un silence');
}
