...
if (start === -1 || end === -1 || end <= start) {
  throw new Error('[unknown-compact] lance install-unknown-command-visual.js avant ce script');
}
...
for (const forbidden of ['buildImmersiveHeader(', '𝐏𝐑𝐄𝐅𝐈𝐗', '𝐌𝐎𝐃𝐄', '𝐈𝐍𝐅𝐎𝐒 𝐒𝐘𝐒𝐓𝐄̀𝐌𝐄']) {
  if (compactBlock.includes(forbidden)) throw new Error(`[unknown-compact] donnée système encore présente: ${forbidden}`);
}
console.log('[unknown-compact] ✅ Style 0 compact sans panneau système');
