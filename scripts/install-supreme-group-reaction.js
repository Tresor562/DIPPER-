...
if (!fs.existsSync(handlerPath)) {
  throw new Error('[supreme-react] handler.js introuvable');
}
...
if (!src.includes(marker)) {
  ...
  if (start === -1 || end === -1 || end <= start) {
    throw new Error('[supreme-react] bloc réaction Supreme introuvable');
  }

  const replacement = `    // ── [SUPREME GROUP REACTION — CLOSED GROUP SAFE] ─────────────\n` +
`    // Prioritaire et indépendante des permissions du bot : même si le groupe\n` +
`    // est en mode annonce/fermé et que le bot n'est pas admin, toute réaction\n` +
`    // au message d'un Supreme Owner est tentée AVANT les gardes de commandes.\n` +
`    // Aucune condition botIsAdmin / botAdminNeeded n'est appliquée ici.\n` +
...
`\n` +
`      // Deux tentatives courtes : utile quand l'événement arrive juste après\n` +
`      // un changement de mode du groupe ou une reconnexion du socket.\n` +
...
`      }\n` +
`\n` +
`      // Diagnostic seulement : ne bloque jamais la réaction sur l'état admin.\n` +
`      if (!reacted) {\n` +
`        try {\n` +
...
`          console.warn(\n` +
`            \`[SupremeReact] réaction refusée après 2 essais | group=\${from} | closed=\${closed} | supremeAdmin=\${senderIsAdmin} | botAdmin=\${botAdminState} | error=\${String(lastReactionError?.message || lastReactionError || 'unknown').slice(0, 160)}\`\n` +
`          );\n` +
`        } catch (_) {}\n` +
`      }\n` +
`    }\n\n`;
...
if (check.status !== 0) {
  throw new Error(`[supreme-react] handler invalide: ${check.stderr || check.stdout}`);
}

const finalSrc = fs.readFileSync(handlerPath, 'utf8');
for (const required of [
  marker,
  'isSuperMe && isGroup && !msg.key.fromMe',
