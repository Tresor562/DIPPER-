...
`  if (mode === 'results') {\n` +
`    cmds.forEach((cmd, i) => { text += s.catCmd({ ...cmd, name: \`\${i + 1}. \${prefix}\${cmd.name}\` }); });\n` +
...
`      text: visual.text, style: visual.style, imageUrl: ctx.imageUrl || null,\n` +
`      quoted: msg, mentions: [ctx.senderJid].filter(Boolean), withImage: false,\n` +
`    });\n` +
`  }\n` +
`  const channelUrl = config.social?.whatsappChannel || 'https://whatsapp.com/channel/0029VbCKhnq7j6gEhuUKMP1V';\n` +
`  const contextInfo = {\n` +
`    mentionedJid: [ctx.senderJid].filter(Boolean), forwardingScore: 1, isForwarded: true,\n` +
`    forwardedNewsletterMessageInfo: {\n` +
`      newsletterJid: config.newsletterJid || '120363411005383995@newsletter',\n` +
`      newsletterName: config.botName || '𝐓𝐇𝐄 𝐁𝐈𝐆 𝐃𝐈𝐏𝐏𝐄𝐑', serverMessageId: -1,\n` +
`    },\n` +
`  };\n` +
`  try {\n` +
...
`        buttons: [{ name: 'cta_url', buttonParamsJson: JSON.stringify({\n` +
`          display_text: '📢 Rejoindre la chaîne', url: channelUrl, merchant_url: channelUrl,\n` +
`        }) }],\n` +
`      }),\n` +
`      contextInfo,\n` +
`    });\n` +
...
`    if (sentMsg?.key?.id) trackMenu(sentMsg.key.id, {\n` +
`      ...ctx, prefix, senderJid: rawSender, mode: 'confirm',\n` +
`      pendingCommandName: fuzzy.confirmCandidate.name, pendingArgs: typedArgs || [], originalMsg: msg,\n` +
...
`    const sentMsg = await sendUnknownCommandVisual(sock, msg, extra, ctx, visual);\n` +
`    if (sentMsg?.key?.id) trackMenu(sentMsg.key.id, {\n` +
`      ...ctx, prefix, senderJid: rawSender, currentCategory: null, currentPage: 1,\n` +
...
`module.exports.handleUnknownCommand = handleUnknownCommand;`;
  ...
  console.log('[unknown-visual] installé');
} else {
...
console.log('[unknown-visual] ✅ style actif + newsletter + CTA + footer du bot');
