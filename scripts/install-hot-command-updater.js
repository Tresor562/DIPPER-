...
if (!serverSource.includes(API_MARKER)) {
  ...
  console.log('[install-hot-updater] api/server.js — endpoint HOT sécurisé installé');
}

for (const file of [indexPath, serverPath, path.join(ROOT, 'utils', 'hotCommandUpdater.js'), path.join(ROOT, 'scripts', 'validate-hot-command.js')]) {
