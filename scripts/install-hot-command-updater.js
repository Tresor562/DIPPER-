...
function replaceRegexOnce(source, regex, replacement, label) {
  ...
  if (matches.length !== 1) {
    throw new Error(`[install-hot-updater] ${label}: attendu 1 occurrence, trouvé ${matches.length}`);
  }
  const singleFlags = regex.flags.replace(/g/g, '');
  return source.replace(new RegExp(regex.source, singleFlags), replacement);
}
...
if (!serverSource.includes(API_MARKER)) {
  ...
  // N'ancre plus l'installation sur le commentaire JSDoc de createServer :
  // les patchs du wrapper peuvent légitimement modifier ce commentaire.
  // La signature de fonction est l'ancre structurelle stable.
  serverSource = replaceRegexOnce(
    serverSource,
    /(^|\n)([ \t]*)function\s+createServer\s*\(\s*\)\s*\{/,
    (match, prefix, indent) => `${prefix}${hotHelpers}${indent}${match.trimStart()}`,
    'helpers API HOT'
  );
  ...
  // Même principe pour les routes : accepter les variations d'indentation
  // introduites par les patchs précédents et injecter juste avant /health.
  serverSource = replaceRegexOnce(
    serverSource,
    /(^|\n)([ \t]*)if\s*\(\s*req\.method\s*===\s*'GET'\s*&&\s*url\.pathname\s*===\s*'\/health'\s*\)\s*\{/,
    (match, prefix, indent) => `${prefix}${routeBlock}${indent}${match.trimStart()}`,
    'routes API HOT'
  );
