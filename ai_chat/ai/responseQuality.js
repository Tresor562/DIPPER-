...
function isLowQualityResponse({ candidate='', userText='', recentAssistant=[] }={}) {
  ...
  // Les phrases de fallback connues ne doivent jamais être prises pour une
  // vraie réponse conversationnelle à une question non triviale.
  if (GENERIC_PATTERNS.some(re => re.test(text)) && String(userText||'').trim().length > 8) return true;

  // Évite les boucles quasi identiques entre tours.
  for (const prev of recentAssistant || []) {
    if (similarity(text, prev) >= 0.82) return true;
  }

  // Pour une question portant sur un sujet nommé, une réponse longue sans aucun
  // recouvrement lexical est suspecte. On reste permissif pour les pronoms et
  // continuations très courtes où le contexte précédent est nécessaire.
