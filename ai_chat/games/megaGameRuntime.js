...
const { enrichGameSpec } = require('./gameContentGenerator');
...
function activeQuiz(td,chatId,ref){if(ref){const e=td.get(chatId,ref);return e?.status==='playing'&&e.gameType==='quiz'?e:null;}const qs=td.list(chatId,{activeOnly:true}).filter(e=>e.status==='playing'&&e.gameType==='quiz');return qs.length===1?qs[0]:null;}
...
async function handleMegaGameMaster(exaucee,{chatId,userId,text,msg,actor={},send}){
  ...
  if (canManage && isDesignIntent(raw)) {
    let spec=designFromText(raw,{by:userId});
    spec=await enrichGameSpec(exaucee.ai,spec);
    const event=td.create(chatId,spec);
    await send(`${describePlan(event)}\n\n✅ J’ai préparé et programmé l’événement.\nID : #${event.alias}\nContenu : ${event.metadata?.contentSource||'plan local'}\nLes inscriptions, manches, chronos, classement et récompenses seront gérés automatiquement.\nPour participer : *je participe #${event.alias}*`);
    return true;
  }

  const quiz=activeQuiz(td,chatId,ref);
  if(quiz && quiz.players?.[userId]){
    const cleaned=raw.replace(/#[a-z0-9_-]{2,32}/i,'').replace(/^\s*(?:reponse|réponse|answer)\s*[:=-]?\s*/i,'').trim();
    if(cleaned){const r=td.submitQuizAnswer(chatId,quiz.id,userId,cleaned);if(r.handled){if(r.duplicate){await send(`Une seule réponse par manche pour #${quiz.alias} 👀`);return true;}await send(r.correct?`✅ Bonne réponse ${mention(userId)} ! +${r.points} pt${r.points===1?'':'s'}.`:`❌ Réponse enregistrée. On attend la fin du chrono.`);return true;}}
  }

  return false;
}

module.exports={handleMegaGameMaster,isDesignIntent,eventListText,rankingText,activeQuiz};
